...
public class SecurityController {
    ...
    public String signInPage(@ModelAttribute("userEnt") SignInDTO user){
        return "signIn";
    }

    @GetMapping("/signUpPage")
    public String signUpPage(@ModelAttribute("userEnt") SignUpDTO user){
        return "signUp";
    }

    @PostMapping("/signUp")
    public String signUp(@ModelAttribute("userEnt") @Valid SignUpDTO sign, BindingResult bindingResult){
        userValidator.validate(sign, bindingResult);
        if(bindingResult.hasErrors()){
            return "signUp";
        }
        try {
            userService.newUser(sign.getPassword(),sign.getUsername());
